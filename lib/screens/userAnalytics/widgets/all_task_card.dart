...
_changeUserTask(String user_id, String task_id){
   DocumentReference _userRef = FirebaseFirestore.instance.collection('users').doc(user_id).collection('tasks').doc(task_id);
}

  @override
  Widget build(BuildContext context) {
        DocumentReference _taskRef = FirebaseFirestore.instance.collection('projects').doc(widget.project.id).collection('tasks').doc(widget.tasks.id);
        DocumentReference _userTaskRef = FirebaseFirestore.instance.collection('users').doc(widget.tasks.user_id).collection('tasks').doc(widget.tasks.user_task_id);
       
    return ListTile(
     onTap: () => Get.to(AllTaskDetailScreen(tasks: widget.tasks, project: widget.project)),
